...
public class ExcelUtils {
	...
	public static String getCellValue(String rowName, String sheetname, String testCaseColName, String colname) {

		sheet = getSheet(sheetname);
		row = getRow(sheet, convertRowNameToNumber(rowName, sheetname, testCaseColName));
		int colindex = convertColumnNameToNumber(sheet, colname) + 1;
		cell = getCell(row, colindex);

		return getCellValue(sheetname, convertRowNameToNumber(rowName, sheetname, testCaseColName) + 1, colindex);
	}

	public static String getCellValue(String rowName, String sheetname, String colname) {
		sheet = getSheet(sheetname);
		int colindex = convertColumnNameToNumber(sheet, colname) + 1;
		int rowIndex = convertRowNameToNumber(rowName, sheetname, Constants.TEST_CASE_COLUMN_NAME) + 1;
		return getCellValue(sheetname, rowIndex, colindex);
	}
	
	public static String getCellValue(String sheetname, String colname) {
		sheet = getSheet(sheetname);
		int colindex = convertColumnNameToNumber(sheet, colname) + 1;
		int rowIndex = convertRowNameToNumber(Config.testCaseId, sheetname, Constants.TEST_CASE_COLUMN_NAME) + 1;
		return getCellValue(sheetname, rowIndex, colindex);
	}
	...
	// Modify remove colume by convert col-name to index
	public static int convertRowNameToNumber(String RowLookupValue, String sheetName, String colname) {
		sheet = getSheet(sheetName);
		int indexCol = convertColumnNameToNumber(sheet, colname);
		int rowLastNumber = getRowNumber(sheetName);
		int indexRow = -1;

		for (int i = 0; i <= rowLastNumber; i++) { // Không có "="
			if (sheet.getRow(i).getCell(indexCol).getStringCellValue().trim().equalsIgnoreCase(RowLookupValue)) {
				indexRow = i;
				break;
			}
		}
